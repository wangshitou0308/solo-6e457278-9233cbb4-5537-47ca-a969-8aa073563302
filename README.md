...
| 主轴 | `M3` 正转、`M5` 停止 |
| 换刀 | `T<n>` 预选刀具、`M6` 换刀（把预选刀设为当前刀，不得同段运动） |
| 注释 | `(…)` 与 `;…`；行号 `N` 忽略 |

**换刀口径（T 预选 / M6 换入）**：机床配置 `tools` 维护刀号及默认 H/D 寄存器，
`initial_tool` 设置初始刀具，`tool_change_point`/`tool_change_tolerance` 给出
机床坐标换刀点与各轴容差。`T` 只预选（可与运动同段），`M6` 才换入；执行 M6 前核对：
预选刀已登记、主轴已停止、固定循环已取消、刀长（G49）与半径补偿（G40 退出）
已取消、主轴基准点落在换刀点±容差内，任一不足即定位 M6 原行整段阻断、保持原刀
与模态。未建立当前刀便切削/钻孔报 `TOOL_NOT_CURRENT`；直线/圆弧/螺旋/钻孔记录
当前 T，生效的 H 或 D 与该刀默认寄存器不一致报 `TOOL_REGISTER_MISMATCH`
（仅提示，不自动替换）。程序包内换刀事件保留预选/换入的真实交错顺序
（M98 L2 重复时 pre#1,change#1,pre#2,change#2）、`repeat_index` 与调用栈。
...
`G28/G30、G54.1、G84-G89、M4/M7-M9` 等；
...
GET    /api/jobs/<id>/report   完整报告；?severity=critical,error
                               ...
                               &wcs=G54,G55&h=H1,H2&d=D1,D2&t=T1,T2
...
换刀报告可按 `t=T1,T2`（也接受 `t=1,2`）筛选：只保留该刀为当前刀期间产生
的问题、轨迹段与孔，`tools` 汇总（刀具表、初始刀、换刀点/容差、T 预选与 M6
换入事件、按 T 的切削长度/钻孔数/换刀次数/问题）只反映命中 T；程序包报告同样
支持 `t` 筛选，事件另带来源程序、`repeat_index` 与调用栈，`trajectory=0`
时筛选仍生效。
...
刀心路径总长 delta 与刀具扫掠包围盒（工件/机床）变化；`tools` 列出刀具表/
初始刀/换刀点/容差是否变化、T 预选与 M6 换入次数、各刀具的切削长度（含固定
循环切削）、钻孔数、换刀次数与问题的新增/解决/净变化。
